...
def conv(image, kernel):
    ...
    for i in np.arange(Hi):
        for j in np.arange(Wi):
            padded_output = padded[i:i+Hk, j:j+Wk]
...
def gradient(img):
    ...
    theta = np.rad2deg(theta) % 360  # Convert to degrees, ensure values are in [0, 360)
...
def canny(img, kernel_size=5, sigma=1.4, high=20, low=15):
    ...
    smoothed_img = conv(img, gaussian)
...
def hough_transform(img):
    ...
    num = int(diag_len * 2.0 + 1)

    rhos = np.linspace(-diag_len, diag_len, num)
    ...
    for x,y in zip(xs,ys):
      for t in range(num_thetas):
          current_rho = x * cos_t[t] + y * sin_t[t]
          rho_pos = np.where(current_rho > rhos)[0][-1]
